<template>
  <div style="padding:30px;">
    <div style="with:100%">
      ...
      <h4 style="color:gray;"> 所選擇時間區間內漫遊吧票卡狀態數量統計 </h4>
      <h5>
        狀態說明：<br>
        <br> WALLET_STATE_INIT= 0<br>
        WALLET_STATE_UPDATE   = 1<br>
        WALLET_FINISH         = 2<br>
        WALLET_STATE_CANCEL   = 3<br>
        WALLET_STATE_TRANSFER = 4<br>
        WALLET_STATE_EXPIRED  = 5<br>
      </h5>
    </div>
...
export default {
  ...
  methods: {
    async refreshList() {
      ...
      this.total = await this.$store.dispatch('datalist/GET_TICKETS_ROAMINGBAR', request)
      this.loading = false
    },
